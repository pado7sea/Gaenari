...
//            wakeLock?.acquire() // WakeLock 활성화
    ...
    private fun initRequestDto() {
        Log.d("Check Dist Service", "ProgramData in InitRequestDto Method : $programData")
//        val timeTargetInfo = TimeTargetInfo(ArrayList())
        ...
        requestDto = SaveDataRequestDto(
            ...
            exerciseType = "P", programType = "D", program = program,
            record = record, speeds = speed, heartrates = heartRate
        )

        Log.d("Check Dist Service", "Init RequestDto : $requestDto")
    }
    ...
    private fun calculateOneMinuteAverages() {
        Log.d("Check Dist Service", "1분 당 평균 값 계산 시작")
    ...
    private fun remainInfoSave(){
        Log.d("Check Dist Service", "조기 종료 시 평균 값 계산 시작")
        val averageSpeed = if (speedCount > 0) oneMinuteSpeed / speedCount else 0.0
        val averageHeartRate =
            if (heartRateCount > 0) (oneMinuteHeartRate / heartRateCount).toInt() else 0
        Log.d(
...
//        wakeLock?.release()
    ...
    inner class LocalBinder : Binder() {
        fun getService(): DistTargetService = this@DistTargetService
    }
     ...
     */
    private fun sendGpsBroadcast(remainDistance: Double, distance: Double, time: Long, speed: Float) {
        val intent = Intent("com.example.sibal.UPDATE_INFO").apply {
            putExtra("remainDistance", remainDistance)
                ...
                when (intent.action) {
                    "com.example.sibal.UPDATE_LOCATION" -> {
                        if(!isPaused) {
                            ...
                            Log.d("Check Dist Service", "Dist Update Location : dist($distance), speed($speed)")
                            // 분 당 속도
