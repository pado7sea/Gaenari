...
public class AfterExerciseServiceImpl implements AfterExerciseService {
  ...
  public void updateProgramUsageCount(String accountId, SaveExerciseRecordDto exerciseDto) {
    if (validateProgramInfo(exerciseDto)) {
      programServiceClient.updateProgramUsageCount(exerciseDto.getProgram().getProgramId())
          .getBody().getData();
    }
  }
  ...
  private Record buildRecord(String accountId, SaveExerciseRecordDto exerciseDto) {
    ...
    Record record = Record.builder()
        ...
        .averagePace(exerciseDto.getSpeeds().getAverage() != 0 ? (3600 / exerciseDto.getSpeeds()
            .getAverage()) : 0)
  ...
  private List<IntervalRangeRecord> buildIntervalRangeRecords(String accountId,
      SaveExerciseRecordDto exerciseDto) {
    List<IntervalRangeRecord> ranges = new ArrayList<>();
    if (exerciseDto.getExerciseType() == ExerciseType.P
        && exerciseDto.getProgram().getIntervalInfo() != null) {
  ...
  private List<Moment> buildMoments(SaveExerciseRecordDto exerciseDto) {
    ...
    if (exerciseDto.getRecord().getTime() != null) {
      for (int i = 0; i < exerciseDto.getSpeeds().getArr().size(); i++) {
        ...
        Integer heartbeat =
            exerciseDto.getHeartrates().getArr().size() > i ? exerciseDto.getHeartrates().getArr()
                .get(i) : null;
  ...
  private List<RecordChallenge> buildRecordChallenges(String accountId,
      SaveExerciseRecordDto exerciseDto) {
  ...
  private List<Integer> fetchNewlyAchievedChallengeIds(RecordAboutChallengeDto challengeDto) {
    ResponseEntity<GenericResponse<List<Integer>>> response = challengeServiceClient.getNewlyAchievedChallengeIds(
        challengeDto);
  ...
  private ProgramDetailAboutRecordDto fetchProgramDetail(Long programId) {
    ResponseEntity<GenericResponse<ProgramDetailAboutRecordDto>> response = programServiceClient.getProgramDetailById(
        programId);
    if (!response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
      throw new ConnectFeignFailException();
    }
  ...
  private Boolean determineFinish(SaveExerciseRecordDto exerciseDto) {
    // 마이크로 서비스간 통신을 통해 프로그램 정보 가져오기
    ProgramDetailAboutRecordDto programDetailDto = fetchProgramDetail(
        exerciseDto.getProgram().getProgramId());
  ...
  // 운동 통계 업데이트
  @Override
  public TotalStatisticDto updateExerciseStatistics(String accountId,
      SaveExerciseRecordDto exerciseDto) {
    // 누적 통계를 찾고, 없다면 첫 기록으로 간주하여 새로운 통계 객체 생성
    Statistic currentStats = statisticRepository.findByAccountId(accountId);
    if (currentStats == null) {
      currentStats = createInitialStatistic(accountId);
    }
  ...
  // 통계 업데이트 메서드
  private void updateStatisticsWithExerciseData(Statistic stats,
      SaveExerciseRecordDto exerciseDto) {
    ...
    double newAveragePace =
        (stats.getPace() * stats.getCount() + averagePace) / (stats.getCount() + 1);
